...
if (isset($_SESSION['id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stmt = $conn->prepare('UPDATE tbl_quiz SET section_id = ?, direction = ?, room_number = ? WHERE room_number = ? AND teacher_id = ? ');
        $stmt->bind_param('isiii', $section_id, $direction, $room_number, $get_room_number, $teacher_id);
        $stmt->execute();
        ...
        while (isset($_POST["item_number_$rowCounter"]) && 
            ...
            $updateStmt = $conn->prepare('UPDATE tbl_quiz SET 
                ...
                WHERE room_number = ? AND item_number = ? AND teacher_id = ?');
            
            $updateStmt->bind_param('isssssssiii', 
            $item_number, $question, $choice1, $choice2, $choice3, $choice4, 
            $correct_answer, $updated_at, $get_room_number, $item_number, $teacher_id);
            $updateStmt->execute();
