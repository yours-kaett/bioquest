    ...
    <body>
        ...
        <main id="main" class="main">
            <div class="d-flex align-items-center justify-content-between">
                ...
                <div>
                    <a href="discussion-add.php?id=<?php echo $get_topic_id ?>">
                        <button class="btn btn-primary">
                            <i class="bi bi-boxes"></i>&nbsp; Setup Discussion
                        </button>
                    </a>
                    <?php
                    $stmt = $conn->prepare(' SELECT * FROM tbl_sub_topics WHERE topic_id = ? ');
                    $stmt->bind_param('i', $get_topic_id);
                    $stmt->execute();
                    $result = $stmt->get_result();
                    if (mysqli_num_rows($result) > 0) {
                        echo '
                        <a href="discussion-edit.php?id=<?php echo $get_topic_id ?>">
